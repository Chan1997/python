...
# data = pd.read_excel(io='D:\Answer_2021.08.09.xlsx')
#
...
querySql = "select * from `baidu_keyword2`"
...
index = 1
for row in list:  # data.values二维数组形式
    if index <=6254 :
        index = index +1
        continue
                ...
                try:
                    ...
                    try:
                        ...
                        if findSourceList !=None and len(findSourceList) >0:
                            ...
                            # for ss in findSourceList:
                            #     if(articleText.find(ss.text)>-1):
                            #         print("含有来源:"+str(ss.text))
                            #         containSource = 1
                            #     else:
                    ...
                    sql = "UPDATE `baidu_keyword2` SET `source` = %s ,`update_time` = SYSDATE() where `query_id` = %s ";
                    # val = ( str(source),queryId)
                    cursor.execute(sql,[source,queryId])
                    conn.commit()
                    success = 1
                    ...
                    # try:
                    #    findSource =  cResult.find_element_by_xpath('//div[contains(@style,"margin-top:-.09rem;")]//div[contains(@class,"single-text")]')
                    #    if findSource != None and str(findSource.text) != None:
                    #        print("含有来源:"+str(findSource.text))
                    #        source = str(findSource.text)
                    # except Exception as ex:
                    #     print("不含有来源")
                    ...
                    if reqNum==2:
                        print("出错了,原因:"+str(ex))
                        sql = "UPDATE `baidu_keyword2` SET `msg` = %s,`update_time` = SYSDATE() where `query_id` = %s  "
                        cursor.execute(sql,[str(ex), queryId])
                        conn.commit()
                    break
            if success ==1:
                print(kw+"完成")
